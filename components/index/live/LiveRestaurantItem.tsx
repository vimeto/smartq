...
import { getRestaurantLiveQueue } from "../../../lib/index/getRestaurantLiveQueue";
...
const RestaurantItem: React.FC<{
  ...
  const livetime = getRestaurantLiveQueue(restaurant);
      ...
      <div className="flex flex-row items-center justify-between">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex flex-row items-center gap-4">
            {livetime !== undefined ? (livetime === -1) ? (
              <>
                <div>{`LIVE`}</div>
                <div className="px-4 py-2 rounded-full bg-gray-300">
                  {`Closed`}
                </div>
              </>
            ) : (
              <>
                <div>{`LIVE`}</div>
                <div
                  style={{ backgroundColor: getQueueTimeBackgroundColor(Number(getRestaurantLiveQueue(restaurant)) || 0) }}
                  className="px-4 py-2 rounded-full text-center">
                  {`${getRestaurantLiveQueue(restaurant)} min`}
                </div>
              </>
            ) : (
              <div>{'Live measuring not configured'}</div>
            )}
          </div>
          <div className="cursor-pointer flex flex-row items-center gap-4" onClick={() => handleModalOpen(restaurant)}>
            {restaurant.queues.length > 0 ? (
              <>
                <div>{`User updated: ${formatDistance(new Date(restaurant.queues[0].updatedAt), new Date(), { addSuffix: true, locale: localeFi })}`}</div>
                <div
                  style={{ backgroundColor: getQueueTimeBackgroundColor(Number(restaurant.queues[0].queueTime) || 0) }}
                  className="px-4 py-2 rounded-full text-center"
                >
                  {`${restaurant.queues[0].queueTime} min`}
                </div>
              </>
            ) : (
